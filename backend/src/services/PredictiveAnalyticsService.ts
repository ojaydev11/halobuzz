...
export class PredictiveAnalyticsService {
  ...
  private calculateConfidence(features: any): number {
    const completeness = Object.values(features).filter(f => f > 0).length / Object.keys(features).length;
    const quality = Object.values(features).reduce((sum: number, f: any) => sum + f, 0) / Object.keys(features).length;
